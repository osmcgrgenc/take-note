...
function App() {
    ...
    useEffect(() => {
    document.title = "Sazik Code"
 }, []);

  const io = require('socket.io-client')
  // URL of server, todo: add dynamic url 
  const socket = io('https://localhost:4000', { path: '/' })
    ...
    <Provider store={store}>
      <BrowserRouter>
        <div className="App">
          ...
          <Route exact path="/:id([A-Za-z]{6})" component={() => <Editor socket={socket}></Editor>} />
          <Route exact path="/register" component={Register} />
          {/* <Route exact path="/" component={() => <MainPage socket={socket} />} /> */}
          <Route exact path="/dashboard" component={() => <Dashboard socket={socket} />} />
          <Switch>
